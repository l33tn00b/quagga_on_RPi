...
- `sudo vtysh`, `configure terminal`, `debug ospf event`, take a look at the log
 ...
 - packets get sent but no packets received (i.e. another router sees hello packets from the one not receiving but hello packets show up in a `tcpdump -i and proto ospf`)? Check the source address of the packets sent by the router not receiving packets. Does it listen on the correct address? 
